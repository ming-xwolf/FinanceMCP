    ...
    async run(args) {
        try {
            ...
            if (marketType === 'crypto') {
                ...
                const ymdToDate = (s) => new Date(parseInt(s.slice(0, 4)), parseInt(s.slice(4, 6)) - 1, parseInt(s.slice(6, 8)));
                const daysBetween = (a, b) => {
                    const da = ymdToDate(a);
                    const db = ymdToDate(b);
                    return Math.max(1, Math.ceil((db.getTime() - da.getTime()) / 86400000) + 1);
                };
                const chooseDaysParam = (needDays) => {
                ...
                if (!ohlcResp.ok)
                    throw new Error(`CoinGecko OHLC 请求失败: ${ohlcResp.status}`);
                if (!volResp.ok)
                    throw new Error(`CoinGecko Volume 请求失败: ${volResp.status}`);
                ...
                const volumesArr = Array.isArray(volJson?.total_volumes) ? volJson.total_volumes : [];
                ...
                let stockData = ohlcData.map(row => {
