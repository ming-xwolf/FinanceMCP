...
- **数据源异构与鉴权复杂**: 多市场/多厂商API字段不一致、鉴权方式各异（Token/Key/Pro Key）。
  - **解决**: 以 MCP 工具为统一抽象层；`AsyncLocalStorage` 每请求上下文透传 `X-Tushare-Token`/`Authorization`/`X-Api-Key`，动态 Getter 提供给各工具，自动选择 CoinGecko Pro/标准/演示 Key。
  - **价值**: 客户端零改造统一调用，服务端按需取用凭证，降低对接与运维成本。

- **技术指标计算易出错**: 默认参数不一致、历史窗口不足导致 NaN、日期对齐混乱。
  - **解决**: 指标参数强制化（如 `macd(12,26,9)`），`dataCalculator` 智能扩展取数窗口，指标模块解耦（MACD/RSI/KDJ/BOLL/MA），结果再裁剪回用户区间。
  - **价值**: 结果稳定可复现，避免“看似可用但数值不对”的隐性风险。

- **分钟级与加密行情接入繁琐**: 频率映射、分页、无效交易对、限流策略容易踩坑。
  - **解决**: `stock_data_minutes` 统一 `1MIN/5MIN/...` 到交易所频率，自动分页至覆盖区间，币对兼容 `BTCUSDT/BTC-USDT/BTC/USDT/coinid.USDT`，并提供明确错误提示。
  - **价值**: 低成本接入分钟线/加密K线，显著降低边界处理代码量。

- **跨协议连接稳定性与可运维性不足**: 传统 SSE 断连与调试困难。
  - **解决**: 提供 Streamable HTTP MCP（JSON-RPC over HTTP）与 SSE 心跳保活，`/health` 健康检查与会话ID管理。
  - **价值**: 高可用、易观测，部署与排障更简单。

- **多市场财务口径不一致**: A股/港股/美股科目命名与结构存在差异，难以横向对比。
  - **解决**: 按市场拆分 `companyPerformance_*` 与 `companyPerformanceDetail_*` 格式器，附综合财务指标与比率计算。
  - **价值**: 同一查询语义下获得统一视图，便于跨市场分析决策。

- **基金等重载查询性能不佳**: 大量字段与多接口串联导致延迟高。
  - **解决**: 字段裁剪、流程优化与缓存策略（工具内优化），实测基金查询性能提升约85%。
  - **价值**: 面向交互式AI与仪表盘的低时延体验。

- **指数成分与估值/财务信息分散**: 需要多次请求并手工拼接。
  - **解决**: `csi_index_constituents` 聚合指数区间行情、成分权重与估值/财务指标，自动回退最近权重/估值日。
  - **价值**: 一次调用拿到“能决策”的摘要，显著提升研究效率。
