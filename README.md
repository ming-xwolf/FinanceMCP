...
flowchart TB
  %% 布局：自上而下
  subgraph Pain[现状痛点（网页搜索直连大模型）]
    P1[可靠性低：结构多变/反爬/限流/广告噪声/时效不可控]
    P2[效率低：多跳检索-解析-清洗，延迟高；口径难复用]
    P3[可溯源差：来源分散、字段不统一，复盘审计困难]
    P4[安全性弱：易触发风控/封禁；缺少权限与速率治理]
  end

  subgraph Method[我们的方法（MCP + 金融数据接口）]
    M1[统一协议：MCP 工具抽象“可调用数据能力”]
    M2[权威数据源：Tushare/交易所/加密行情，结构化可版本化]
    M3[高效稳健：请求上下文透传 Token；指标智能扩窗；HTTP/SSE 保活与超时]
    M4[可审计：固定入参/出参，便于复现、对比与合规留痕]
  end

  subgraph Gain[直接收益]
    G1[更高准确一致：统一财务与指标口径，告别“抓取+正则”]
    G2[更低时延成本：少跳数直达权威接口，计算一次到位]
    G3[更好安全合规：令牌治理、速率控制、服务端隔离]
    G4[更易扩展：新增数据能力=新增工具，前端/Prompt 无需改造]
  end
